...
// Spesial-funksjoner for USART3 ----------------------------------------------------
/*
// Initialiserer USART3 
void usart_usb_init(void) {
	usart_init(&USART3, &PORTB, PIN0_bp, PIN1_bp, 1667); // start USART3 på PORTB med 9600 baud
}

// Sender en hel tekststreng via USART3
uint8_t usart_usb_transmit_char_array(const char string[]) {
	return usart_transmit_string(&USART3, string); // sender streng via USART3
}

// Sender et heltall via USART3
void usart_usb_transmit_uint16(uint16_t value) {
	usart_transmit_uint16(&USART3, value); // sender heltall via USART3
}

// Mottar ett tegn via USART3
char usart_usb_receive_char(void)
{
	while (!(USART3.STATUS & USART_RXCIF_bm)); // vent til et tegn er mottatt
	return USART3.RXDATAL; // les og returner mottatt tegn
}

// Mottar en tekststreng via en USART til buffer
void usart_usb_receive_string(char *buffer, uint8_t max_length)
{
	usart_receive_string(&USART3, buffer, max_length);
}
*/
